<template>
  <main class="container">
    <div class="userContent">
      <div class="block" style="display: inline-block; float: left; width: 340px">
        <el-tree
          :data="data"
          node-key="id"
      ...
      <div style="display: inline-block; width: 828px">
        ...
        <el-table class="userListTable" :data="tableData">
          ...
          <el-table-column :label="$t('views.userList.operate')" width="100px" fixed="right">
            <template slot-scope="{ row }">
    ...
    <el-dialog
      ...
      top="8vh"
      :title="isAdd ? $t('views.userList.addUser') : $t('views.userList.editUser')"
    >
      <el-form
        ref="ruleForm"
        label-width="150px"
      ...
      >
        ...
        <el-form-item v-if="userForm.uid" :label="$t('views.userList.password')">
          <el-input
            v-model="userForm.password"
            class="addUserInput"
            show-password
        ...
        <el-form-item>
          <el-button type="text" class="submitBtn" size="small" @click="userAdd">{{
            $t("views.userList.submit")
          }}</el-button>
          <el-button type="text" class="cancelBtn" size="small" @click="cancelAdd">{{
...
export default class DepartmentList extends VueBase {
  ...
  private async getTableData() {
    ...
    this.loadingStart();
    const { status, msg, data, total } = await this.services.user.userList(params);
    this.loadingDone();
    if (status !== 201) {
  ...
  private async departmentList() {
    this.loadingStart();
    const { status, msg, data } = await this.services.department.departmentList();
    this.loadingDone();
    if (status !== 201) {
      this.$message.error(msg);
      return;
  ...
  private append(node: any, data: { id: number; label: string; children: [{}] }) {
  ...
  private async remove(node: any, data: any) {
    // 执行删除
    const { status, msg } = await this.services.department.departmentDel(data.id);
  ...
  private async submitEdit(node: any, nodeData: any) {
    if (nodeData.label == this.currentDepartmentName) {
      ...
      if (this.newDepartment) {
        // 创建部门
        const { status, msg, data } = await this.services.department.departmentAdd(
          params
        );
        if (status !== 201) {
